function Radar(radarContainer) {
    ...
    this._changingNumberContainer = this._container.querySelector(
      ".changing-number-container"
    );
    // 不断变化的数字
    this._digits_base = Math.pow(
      10,
      Math.min(this._changingNumberContainer.dataset.number.length, 16)
    );
    // 数字位数，最大为16
  
    this._mapTranslateZ = (function (container) {
      // 相机漫游时拉近的距离
      var fontSize = parseInt(getComputedStyle(container).fontSize);
      return (300 / 16) * fontSize;
    })(this._container);
  
    this._scaleFactor = this._radarMap.offsetWidth / this._containerWidth;
    // 为了使地图拉近之后不失真而放大的倍数
  ...
  Radar.prototype._addOneRiskPoint = function (options) {
    ...
    elements_group.innerHTML =
      ...
      '</div>\
                                      <div class="info-content">\
                                          <div>\
                                              风险发生地：<span class="risk-city">' +
      options.city +
      '</span>\
                                          </div>\
                                          <div>\
                                              风险卡片：<span class="risk-num">' +
      options.total +
      "</span>\
                                          </div>\
                                      </div>\
    ...
    dashed_circle.style.backgroundImage =
      "url(" + this._getDashedCircleBg(dashed_circle.offsetWidth) + ")";
  
    // 计算和设置圆圈、红旗、信息面板的位置
    var dashed_circle_left =
        point_x * this._scaleFactor - dashed_circle.offsetWidth / 2,
      dashed_circle_top =
        point_y * this._scaleFactor - dashed_circle.offsetHeight / 2,
    ...
    this._riskElements.push({
      ...
      translate: [
        (this._containerWidth * 0.5 - point_x) * this._scaleFactor,
        this._calcTYByTZ(this._mapTranslateZ, dashed_circle.offsetHeight / 2) -
          point_y * this._scaleFactor
      ],
      rotateY: (function (riskElements) {
        // 旋转的角度正负零交替
        var base_deg = 10,
          max_deviation_deg = 5;
        var pn = (riskElements.length % 2) * 2 - 1;
        // positive or negative
        var new_base_deg =
          Math.ceil(((riskElements.length + 1) % 3) / 2) * base_deg;
        // 10 10 0 10 10 0 ...
        return (
          pn * (new_base_deg + Math.round(Math.random() * max_deviation_deg))
        );
      })(this._riskElements),
    ...
    info_panel.style.transform =
      getComputedStyle(info_panel).transform + " rotateY(" + -rotate_deg + "deg)";
  };
  ...
  // 地理坐标转换成屏幕坐标
  Radar.prototype._geoCoordsToScreenCoords = function (geoCoords) {
    // geoCoords:经纬度数组
    var china_geometry_bounds = [
      73.4994136,
      53.56097399999999,
      61.2733963,
      35.40335839999999
    ...
    // 西北的经纬度和经纬度跨度
    var point_x =
        (Math.abs(geoCoords[0] - china_geometry_bounds[0]) /
          china_geometry_bounds[2]) *
        this._containerWidth,
      point_y =
        (Math.abs(geoCoords[1] - china_geometry_bounds[1]) /
          china_geometry_bounds[3]) *
  ...
  Radar.prototype._calcRiskPointIndex = function (point_x, point_y) {
    var point_offset_x = point_x - this._centerX, // 与中心点的偏移
      ...
      riskPointRadian = -Math.atan(point_offset_y / point_offset_x);
    // 风险点在雷达扫描圆形的弧度
    if (point_offset_x < 0) {
    ...
    var p = parseInt(getComputedStyle(this._container).perspective), // 原始视角
      ...
      tr_o_offset_y =
        ...
        (document.body.clientHeight -
          ...
          point_screen_bottom_offset);
    // 最后相机要拉近到的y值与transform origin中心点的偏移量
  ...
  Radar.prototype._calcOffsetBeforeRotateX = function (
    newOffset,
    rotateX,
    oldPerspective
  ) {
    ...
    return (
      (oldPerspective * x1) /
      (oldPerspective + x1 * Math.sin((Math.PI / 180) * rotateX))
    );
  };
  ...
  Radar.prototype._rotateRiskPointMask = function () {
      ...
      this._tmpRiskPointsArr.forEach(function (point) {
        if (that._maskCtx.isPointInPath(point.x, point.y)) {
          ...
          if (!that._riskPoints[riskPointIndex]) {
            var riskPointGroup = document.createElement("div");
            // 相同索引的风险点放在一组
            riskPointGroup.className = "risk-point-group";
          ...
          point._willDelete = true;
          // 将要删除的标记
        }
      });
  
      // 遍历完之后删除已处理过的风险点
      this._tmpRiskPointsArr = this._tmpRiskPointsArr.filter(function (
        pointItem
      ) {
    ...
    this._riskPoints[riskPointIndex] &&
      this._riskPoints[riskPointIndex].classList.add("flashing");
  ...
  Radar.prototype._getDashedCircleBg = function (radius) {
    ...
    var solid_dash_len =
        (Math.PI * (radius - ctx.lineWidth)) / dash_cnt / (1 + dash_ratio),
  ...
  Radar.prototype._handleCurRiskElements = function () {
    ...
    if (curRiskElements) {
      ...
      var lastRiskElements = this._riskElements[
        (this._curRoamingIndex - 1 + this._riskElements.length) %
          this._riskElements.length
      ];
      if (lastRiskElements) {
  ...
  Radar.prototype._riskPointsRoaming = function () {
    ...
    var radarMapStyle =
      ...
      this._mapTranslateZ +
      "px) rotateY(" +
      curRiskElements.rotateY +
      "deg) rotateX(" +
      this._rotateX +
      "deg) translate3d(" +
      curRiskElements.translate[0] +
      "px, " +
      curRiskElements.translate[1] +
      "px, 0)";
    ...
    // 当前的信息面板设置透明度
    var roamingDuration =
      this._roamingDuration ||
      parseFloat(
        getComputedStyle(this._radarMap).transitionDuration.split(", ")[0]
      );
    setTimeout(
      function (that) {
  ...
  Radar.prototype._initEvent = function () {
    ...
    this._riskPointsContainer.addEventListener(
      "animationend",
      function (e) {
    ...
    this._radarMap.addEventListener(
      "transitionend",
      function (e) {
        ...
        if (e.target === e.currentTarget) {
          if (that._isLyingDown()) {
            ...
            if (that._isInRoamingState()) {
              // 相机漫游状态
              that._handleCurRiskElements();
              // 当前风险点漫游结束
              that._curRoamingIndex =
                (that._curRoamingIndex + 1) % that._riskElements.length;
          ...
          e.target.classList.contains("red-flag") &&
          !e.target.parentElement.nextElementSibling
        ) {
          // 最后一根红旗树立起来之后开始漫游
          that._radarMap.classList.add("roaming");
          // 修改transition duration和transition timing-function
          setTimeout(function () {
            that._riskPointsRoaming();
          }, 1000);
        }
